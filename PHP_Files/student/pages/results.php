...
function getStudentSemesters($student_id) {
    global $connection;
    $stmt = $connection->prepare("SELECT DISTINCT r.semester_id, s.semester_name FROM result r JOIN semester s ON r.semester_id = s.semester_id WHERE r.student_id = ? AND r.status IN ('published', 'verified') ORDER BY r.semester_id");
    $stmt->bind_param("s", $student_id);
    $stmt->execute();
...
@media print {
    ...
    /* Container full width - NO SCROLLBAR */
    .container-fluid,
    .col-lg-10,
    .col-md-9 {
        width: 100% !important;
    ...
    /* STUDENT DETAILS - SIMPLE - ONLY NAME AND ID */
    .student-info-box {
    ...
    /* Adjust column widths to fit all content */
table.table th:nth-child(1) { width: 4% !important; }   /* # */
table.table th:nth-child(2) { width: 10% !important; }  /* Code */
table.table th:nth-child(3) { width: 30% !important; }  /* Subject */
table.table th:nth-child(4) { width: 6% !important; }   /* Credits */
table.table th:nth-child(5) { width: 7% !important; }   /* Obtained */
table.table th:nth-child(6) { width: 7% !important; }   /* Total */
table.table th:nth-child(7) { width: 10% !important; }  /* Percentage */
table.table th:nth-child(8) { width: 11% !important; }  /* Grade */
table.table th:nth-child(9) { width: 15% !important; }  /* Status (extra space for footer) */
...
<div class="container-fluid py-4">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="d-flex align-items-center flex-wrap gap-3">
                            <div><small class="text-muted">Paid:</small><div class="h5 mb-0">NPR <?php echo number_format($payment_check['amount_paid'], 2); ?></div></div>
                            <div><small class="text-muted">Total:</small><div class="h5 mb-0">NPR <?php echo number_format($payment_check['total_amount'], 2); ?></div></div>
                            <div class="ms-auto"><a href="payments.php" class="btn btn-primary"><i class="bi bi-credit-card me-2"></i>Pay Now</a></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <!-- Semester Filter Row (hidden in print) -->
    <div class="row mb-4 align-items-center">
        <div class="col-lg-9">
            <div class="card">
                <div class="card-body">
                    <h6 class="text-muted mb-3"><i class="bi bi-filter me-2"></i>Available Semesters</h6>
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        while($sem = $semesters->fetch_assoc()): 
                            $sem_count++;
                        ?>
                            <span class="badge bg-primary bg-opacity-10 text-primary border border-primary px-3 py-2" style="font-size: 1rem;">
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body p-0">
                    <?php if($payment_check['payment_status'] === 'Paid'): ?>
                        ...
                        <!-- STUDENT DETAILS - SIMPLE - ONLY NAME AND ID (hidden on screen) -->
                        ...
                        <!-- RESULT SUMMARY LINE (visible in print) -->
                        <?php
                        $total_obtained = 0;
                        $total_max = 0;
                        $results->data_seek(0);
                    ...
                    <div class="text-center py-5">
                        ...
                        <p class="text-muted mb-4">Complete fee payment to view your results.</p>
